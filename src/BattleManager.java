...
public class BattleManager {
  ...
  private int playerMaxDamage; // not a final variable as it can change
                               // depending on player's weapon.
   ...
   * 
   * <p>
   * Side-effect sets the player and monster of the battle.
   * 
  ...
  /**
   * Main loop for the battle. Simulates the player and monster turns. If either
   * the player or monster's health is reduced to 0, the battle ends.
   * 
   * <p>
  ...
  /**
   * This method simulates the attack phase against a defender.
   *
   * NOTE: This would be private however it needs to be public for the sake of
   * the testing phase of the coursework :)
   * 
  ...
  public void attackPhase(Actor attacker, Actor defender, int damage) {
    if (damage == 0) {
      ...
      System.out.println(
          attacker.getName() + " landed a hit for " + damageDone + " HP!");
    }
  }
  ...
  /**
   * Gives the player their rewards after winning the battle
   *
   * <p>Side-effect changes inventory, experience and gold of the player.
   */
